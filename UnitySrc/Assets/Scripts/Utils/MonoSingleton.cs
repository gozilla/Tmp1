...
namespace Project.Utils
{
	...
	public class MonoSingleton<T> : MonoBehaviour where T : MonoSingleton<T>
	{
		...
		protected virtual void Update()
		{
			...
			if (singletonAttribute == null && _levelId != SceneManager.GetActiveScene().buildIndex)
				Destroy(gameObject);
		}
		...
		private static string COMPANY_NAME = "None";
			...
			get
			{
				if (_instance == null)
				{
					...
					if (_instance == null)
					{
						...
						_instance = gameObject.AddComponent<T>();
						DontDestroyOnLoad(_instance);
						Debug.Log("DontDestroyMonoSingleton AddComponent " + typeof(T).Name + " _instance = " + _instance);

						// must be root object
						gameObject.transform.parent = null;// GetBaseObject().transform;
					}
				}
