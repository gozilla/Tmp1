...
namespace OdsReadWrite
{
    internal sealed class MainWindowModel : INotifyPropertyChanged, IDataErrorInfo
    {
        private const string
            ...
            SheetsPropertyName = "Sheetss",
            RowsPropertyName = "Rows",
            ColumnsPropertyName = "Columns",
			//ProgressBarPropertyName = "ProgressBar",
		...
		private readonly DelegateCommand writeLocaleCommand;
		...
		//private int progress = Settings.Default.Progress;
        ...
        public MainWindowModel()
        {
			...
			this.writeLocaleCommand = new DelegateCommand(this.Write, this.CanWrite);
		...
		public DelegateCommand WriteLocaleCommand
		{
			get { return this.writeLocaleCommand; }
		}
		...
		//	set { this.SetValue(ProgressBarPropertyName, ref this.progress, value); }
		//}
            ...
            get
            {
                switch (propertyName)
                {
                    ...
                    case OutputPathPropertyName:
						...
						this.WriteLocaleCommand.InvalidateCanExecuteChanged();
        ...
        private void Read()
        {
            if (this.CanCreate())
            {
                ...
                dlg.FileName = "GameLocalize"; //default file name
                dlg.DefaultExt = ".txt"; //default file extension
                dlg.Filter = "Localize documents (.txt)|*.txt"; //filter files by extension
                ...
                if (result == true)
                {
                   ...
                   try
                    {
                        new OdsReaderWriter().WriteLocaleFile(this.Data, filename, this.InputPath);
        ...
        private void Write()
        {
            if (this.CanWrite())
            {
				...
				new OdsReaderWriter().WriteLocaleFile(this.Data, this.OutputPath, this.InputPath);
        ...
        private void ShowSaveFileDialog()
        {
            var saveFileDialog = new SaveFileDialog()
            {
                ...
                DefaultExt = "*.ods|*.txt",
                Filter = "Open Document Spreadsheet (.ods)|*.ods|Open Document Spreadsheet (.txt)|*.txt"
			};
