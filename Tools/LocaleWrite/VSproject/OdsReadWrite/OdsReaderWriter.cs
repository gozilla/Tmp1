...
namespace OdsReadWrite
{
    internal sealed class OdsReaderWriter
    {
		...
		public void WriteLocaleFile(DataSet odsFile, string outputFilePath, string inputFilePath)
		{
			...
			using (StreamWriter sw = File.CreateText(outputFilePath))
			{
				...
				foreach (DataTable sheet in odsFile.Tables)
				{
					...
					if (sheet.ToString() == "fonts")
					{
						...
						{
							object[] cells = rows[i].ItemArray;
                            if (cells.Length >= 2 && cells[0].ToString() != "" && cells[1].ToString() != "")
                            {
								if (firstLine)
								{
									firstLine = false;
								...
								allCells += "				" + "\"" + cells[0].ToString() + "\":" + "\"" + cells[1].ToString() + "\"";
                                
                            }
						}
						sw.WriteLine(allCells);
						sw.WriteLine("	},");
					}
					else if (sheet.ToString() == "fontsCustomMaterials")
					{
						sw.WriteLine("	" + "\"fontsCustomMaterials\":");
						sw.WriteLine("	{");
						bool firstLine = true;
						string allCells = "";
						for (int i = 0; i < rows.Count; i++)
					...
					else if (sheet.ToString() == "locales")
					{
						sw.WriteLine("	" + "\"locales\":");
						sw.WriteLine("	{");
						bool firstLine = true;
						string allCells = "";
						for (int i = 0; i < rows.Count; i++)
					...
					else
					{
						...
						sw.WriteLine("	" + "\"" + sheet.ToString() + "\":");
						sw.WriteLine("		{");
						sw.WriteLine("			\"localizeItems\":");
						sw.WriteLine("			{");
						bool firstLine = true;
						string allCells = "";
						for (int i = 0; i < rows.Count; i++)
						{
                            ...
                            //{
                            //	allCells = allCells + ":"+ cells[j].ToString();
                            //}
                            //allCells += ",";
                            if (cells.Length >= 2 && cells[0].ToString() != "" && cells[1].ToString() != "")
                            {
							...
							//progress = (i * 10).ToString();
						}
						sw.WriteLine(allCells);
						sw.WriteLine("			}");
						sw.Write("		}");
						//progress = sheetCount.ToString();
					}
