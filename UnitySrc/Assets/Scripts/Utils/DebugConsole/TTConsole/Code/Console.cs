...
#region -------------Singleton Setting------------------
			...
			get
			{
				...
				lock (_lock)
				{
					if (_instance == null)
					{
						...
						if (_instance == null)
						{
							_instance = FindObjectOfType(typeof(Console)) as Console;

							if (_instance == null)
							{
								GameObject console = new GameObject("(singleton) " + typeof(Console).ToString());
								_instance = console.AddComponent<Console>();
		...
		public static bool isConsoleAllowed = true;
		...
		static int maxLinesForDisplay = 5000;
		static int maxLinesForAsyncRecord = 5000;
...
#region Message Struct
		...
		struct Message
		{
			...
			public static Message Unity(object message, Color unityColor)
			{
...
#region Mono & base
		...
		void Init()
		{
			...
			this.RegisterCommandCallback("clear", CMDClear, "Clear All Log");
			this.RegisterCommandCallback("sys", CMDSystemInfo, "Show System Info");
			this.RegisterCommandCallback("?", CMDHelp, "Show All CMD");
			this.RegisterCommandCallback(":", CMDShowMessageType, "Control Show Log Type (-1:all,0:normal,1:warn,2:error,3:system,4:input,5:output,6:unitylog)");
			this.RegisterCommandCallback("fps", CMDShowFPS, "Fps Conrtol");
			this.RegisterCommandCallback("mem", CMDShowMemory, "Memory Detail Show");
			this.RegisterCommandCallback("log", CMDStartAsyncLoging, "Start Asyn Loging");
			this.RegisterCommandCallback("stack", CMDEnableStackTrace, "Stack");
			this.RegisterCommandCallback("clean_cache", CMDCleanCache, "Clean Asset Bundles Cache");

			enableStackTrace = PlayerPrefs.GetInt("enableStackTrace", 0) != 0;
		...
		private bool isLogMessageRegistered = false; 
		...
		void HandleUnityLog(string condition, string stackTrace, LogType type)
		{
			...
			if (enableStackTrace)
				message += "<i>" + "<color=#" + ColorToHex(Color.grey) + ">" + "<size=9>" + stackTrace  + " </size>" + " </color>" + "</i>";


			Color unityColor = new Color(0.3882f, 0.7725f, 1f, 1f);
			...
			LogMessage(Message.Unity(message, unityColor));
...
#region DrawWindows
		...
		void DrawingContent()
		{
			...
			endMsgIndex = Math.Min(endMsgIndex, GetAllCurrentTypeMessages().ToArray().Length-1);
...
#region Console commands
		...
		object CMDCleanCache(string[] args)
		{
#if UNITY_2017
			return "Cant Clean Cash";
#else
			Caching.CleanCache();
			return "Cash Clean";
#endif
		...
		object CMDEnableStackTrace(string[] args)
		{
			if (args == null || args.Length == 1)
			{
				return "Type stack 1 or stack 0";
			}
			else
			{
				int i = 0;
				if (int.TryParse(args[1], out i))
				{
					...
					PlayerPrefs.SetInt("enableStackTrace", i);
				}
			}
			return "";
		}
		...
		object CMDShowMemory(string[] args)
		{
			...
			{
				Log(textures[i].name + " : " + (UnityEngine.Profiling.Profiler.GetRuntimeMemorySizeLong(textures[i]) * mbseed).ToString("#0.##") + "mb", MessageType.OUTPUT);
			}
			...
			{
				Log(clips[i].name + " : " + (UnityEngine.Profiling.Profiler.GetRuntimeMemorySizeLong(clips[i]) * mbseed).ToString("#0.##") + "mb", MessageType.OUTPUT);
			}
		...
		object CMDStartAsyncLoging(string[] args)
		{
			...
			{
				StartAsyncWriteLog();
				return "Start Async Log";
			}
			else
			{
				return "Stop Async Log";
			}
		}
...
#region Thread Writing Log
	...
	public class MemoryCounter
	{
		...
		public bool Update(int updateInterval)
		{
			...
			allocated = UnityEngine.Profiling.Profiler.GetTotalReservedMemoryLong();
